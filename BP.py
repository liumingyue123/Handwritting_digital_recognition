...
def backward(M, W, b, net, O, y, E, label):
    """
    后向推导函数
    :param M:层数向量
    :param W:权值矩阵
    ...
    for m in layer: #从输出层回退
        if m == len(M) - 1:  # 如果是输出层
            ...
            W[m-1] -= learningrate * grad[m] *O[m-1].transpose()
            b[m-1] -= learningrate * grad[m]
        else:
...
def training(M , W , b, dataMat , labelMat , testMat , labelMat2 , iteration = 5 ):
    ...
    for iter in range(iteration):
        ...
        for i in range(len(dataMat)):
            ...
            W , b = backward(M , W , b, net , O , y ,E , labelMat[i].transpose())
            #print("第%d个样本训练！"%i)
...
if __name__ == "__main__":
    ...
    # b = grab('biases2.txt')
    # W = grab('weights2.txt')
    dataArr, labelArr , testArr , labelArr2 = test_loadimage.getdata(16)
    dataMat = mat(dataArr)
